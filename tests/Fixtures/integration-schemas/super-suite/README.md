...
| `suite_kits` | An **item-picker** over a JSON column: the field is APPENDED (`$merge` … `$append`), its catalog is a `type: custom` splash source (one name carries an apostrophe), the create/edit splash routes need non-empty `constants`, and `json_rules` names every key of the stored row | v3.5.27 (the form bound an undefined catalog and crashed on mount; a quote in the data was a PHP parse error; the splash services were never generated; the Quantity field of the picker's modal was never rendered) |
| `suite_invoices` → `suite_invoice_lines`, `suite_invoice_notes` | The remaining **inline_items** variants: the default card variant with `can_delete: false` and `inject_from_parent` (`currency`), a second inline entry on the same parent, per-row validation, and a row that must not set columns it does not declare | v3.5.27 (inline rows were unvalidated: with `guarded = []` a client chose the child's id and timestamps, and a missing required field was a 500) |
| `suite_proforma_invoices` → `suite_proforma_invoice_items` | A **create wizard whose middle step is an inline-items table** (`proforma_items` in a step's `field_keys`, with the `sync_to` total beside it): rows survive Back/Next, the Review & Confirm step says "N item(s)", one submit saves the proforma and every row, and each Next drafts the form including the rows | v3.5.28 (every text input in an inline-items or item-picker modal was invisible: the generator left a bare `<template>` around it) |
