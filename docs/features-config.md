...
#### Custom views over a generated list

::: tip Since v3.5.21
Verified directly against `src/Generators/Templates/backend/Features/list/service.stub`
and `tests/Unit/Generators/Backend/Services/ListServiceGeneratorTest.php`.
:::

A hand-written wrapper service that presents a custom view over a module's data (a
fleet board, a status-chip row, a cross-module report) used to re-walk
`$result['data']['data']` by hand to add fields — and that enrichment silently
vanished the moment the same wrapper also needed an export, since export followed
an entirely separate code path with no hook of its own.

Every generated `{Module}ListService::execute()`/`export()`/`process()` now accepts
an optional last `?callable $enrich` parameter, forwarded through to the consuming
app's `ListServiceTrait::processListQuery()`/`exportData()`:

```php
{Module}ListService::execute($data, query: $scoped, enrich: [self::class, 'enrich']);
```

- `$enrich` is `fn (array $rows, array $data): array` — it must return exactly one
  row per input row (the trait throws `\LogicException` otherwise). Narrow rows
  with the pre-scoped `$query`, not inside the enricher: pagination `meta` is
  already computed by the time it runs.
- Prefer a batch lookup over the whole page inside the enricher, not one query per
  row.
- Define the enricher method in a hand-owned service — never inside the generated
  `{Module}ListService.php` itself, which is rewritten wholesale on `--force` (no
  hand regions exist in this file, unlike `Routes/api.php`/`Controller.php`).
- `enrich:` as a named argument requires a `{Module}ListService` generated by
  v3.5.21 or later — passing it against an older generated file raises an
  "Unknown named parameter" error, since the parameter doesn't exist there yet.
- For aggregates over the same scope a list applies (a total row count, a
  breakdown), use the consuming app's own `ListServiceTrait::scopedQuery()`/
  `listCounts()` (added alongside this in the same app-side change) rather than
  re-deriving the scope by hand.

**Not retroactive** — an existing generated `{Module}ListService.php` gains the
`$enrich` parameter only when regenerated against v3.5.21+.
...
| `option_label` | string | Key read from each option object for its display text — `Select2Field`'s underlying component reads `option[optionLabel]` with **no fallback**. Defaults to `'name'`, which fits an api-select's API-fetched `{id, name}` records; a plain `select` with a hand-authored static `options: [...]` array must set this explicitly whenever its objects don't use that key (e.g. `{value, label}` pairs — see `option_value` below) or every option silently displays/binds `undefined`. |
| `option_value` | string | Key read from each option object for its submitted value — same no-fallback `option[optionValue]` read, defaulting to `'id'`. Same static-`select` requirement as `option_label`: for `options: [{"value": "REFUNDED", "label": "Refund"}, ...]`, set `"option_value": "value", "option_label": "label"` or the field submits `undefined` on every selection (confirmed live 2026-08-26: a hand-authored `current_period_decision`/`deposit_decision` select on a required field submitted nothing at all, failing "field is required" no matter what the user clicked — the click worked, the bound value never did). |
...
| `select` (static options, no `splashKey`) | A hand-authored dropdown: `options: [{...}, {...}]`, any object shape. **Set `option_value`/`option_label`** to match your objects' actual keys (see the field-shape table above) — the default `id`/`name` fits an FK-backed select, not a hand-authored one, and there's no fallback: a mismatched key submits `undefined` on every selection, not an error. |
| `color`, `password`, `time`, `item-picker`, `inline-items` | Also real, each with its own stub under `src/Generators/Templates/frontend/fields/` — not detailed here; inspect the stub or an existing generated field of that type for the exact shape. |
